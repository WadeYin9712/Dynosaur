...
def truncate_metadata(metadata, max_len):
    truncated = {}
    for i in metadata:
        if i in ['multilinguality', 'data_fields', 'supported_task', 'task_category', 'task_ids']: 
            continue
        if i != 'selected_data':
...
instruction = 'Given a dictionary containing a dataset description and a few examples, our goal is to design up to three different tasks based on this dataset. Each task should still be a dictionary, including the instruction, input fields and one output field. The following are two examples.\n\nExample 1:\nInput:\n'
instruction += str(truncate_metadata(demonstrations['squad']['data'], 1000))+ '\n\nTasks:\n' + str(demonstrations['squad']['tasks']) + '\n\nExample 2:\nInput:\n'
instruction += str(truncate_metadata(demonstrations['anli']['data'], 1000)) + '\n\nTasks:\n' + str(demonstrations['anli']['tasks'])
...
prediction_filename = '../../instruction_data/instruction_w_description'
prediction = {}
